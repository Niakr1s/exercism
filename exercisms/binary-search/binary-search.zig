...
pub fn binarySearch(comptime T: type, target: T, items: []const T) ?usize {
    return switch (items.len) {
        0 => null,
        1 => if (items[0] == target) 0 else null,
        else => blk: {
            const mid = items.len / 2;
            if (target >= items[mid]) { // right
                const found = binarySearch(T, target, items[mid..]);
                break :blk if (found != null) found.? + mid else null;
            } else { // left
                const found = binarySearch(T, target, items[0..mid]);
                break :blk if (found != null) found.? else null;
            }
        },
    };
}
